...
class ShopifyProductBackend implements ProductBackend {
  ...
  async checkInventory(
    ...
    return products
      ...
      .map((product: Product) => {
        ...
        const totalQuantity = variants.reduce((sum: number, v: ProductVariant) => {
          const qty = v.availability?.quantity ?? 0;
          return sum + qty;
        }, 0);
        const inStock = variants.some((v: ProductVariant) => v.availability?.inStock ?? false);
  ...
  async getRecommendations(
    ...
    if (strategy === 'similar' && sourceProducts.length > 0) {
      const categories = sourceProducts.map((p: Product) => p.category).filter(Boolean);
    ...
    return result.products
      .filter((p: Product) => !productIds?.includes(p.id))
      .slice(0, limit)
      .map((product: Product) => ({
        product,
  ...
  private mapProduct(p: ShopifyProduct): Product {
    ...
    return {
      ...
      variants,
      availability: {
        inStock: p.availableForSale ?? variants.some((v: ProductVariant) => v.availability?.inStock ?? false),
        quantity: variants.reduce((sum: number, v: ProductVariant) => sum + (v.availability?.quantity ?? 0), 0),
      },
    };
  }
}
...
class ShopifyCartBackend implements CartBackend {
  ...
  async addToCart(
    ...
    if (!variantId) {
      ...
      const availableVariant = product.variants?.find((v: ProductVariant) => v.availability?.inStock);
      if (!availableVariant) {
  ...
  async clearCart(cartId: string): Promise<Cart> {
    ...
    // Remove all items
    const lineIds = cart.items.map((item: CartItem) => item.id!);
    for (const lineId of lineIds) {
      await this.removeFromCart(cartId, lineId);
    }

    // Return updated cart
    return (await this.getCart(cartId)) ?? cart;
  }
